...
const OptimizationFilterBar = () => {
  ...
  const formatDateForNav = () => {
    ...
    // For year view
    if (interval === 'year') {
      return to.getFullYear().toString();
    }
  ...
  const setInterval = (interval: 'day' | 'week' | 'month' | 'year') => {
    setFilters({ interval });
    setShowIntervalDropdown(false);
  };
          ...
          <div className="absolute z-10 mt-1 w-48 bg-black rounded-md shadow-lg py-1">
            {INTERVAL_OPTIONS.map(option => (
              <button
                ...
                onClick={() => setInterval(option.value as 'day' | 'week' | 'month' | 'year')}
              >
                {option.label}
...
export default OptimizationFilterBar; 